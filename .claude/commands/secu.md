...
Task({
  name: "security",
  prompt: "Lis .claude/agents/context/TEAMMATES_PROTOCOL.md puis .claude/agents/security.template.md (et .claude/agents/security.md s'il existe — adaptations projet). Tu fais partie de TEAM-Buzz sur BuzzControl. Mets-toi en IDLE après avoir envoyé ACTIF — le teamleader t'enverra ta tâche."
})
```
...
Spec : `.claude/agents/security.template.md` (+ `.claude/agents/security.md` si présent)
