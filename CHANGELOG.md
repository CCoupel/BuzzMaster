...
- **Backstage page consolidates game prep configuration (#215)** — Nouvelle page `/admin/backstage` regroupant les trois zones de préparation de partie (Quiz méta, Entracte, Fonds d'écran) en 3 onglets. La page Quiz passe elle aussi à 2 onglets (Questions / Rafale), `RafalePage` étant embarquée dans l'onglet Rafale. Redirection historique `/admin/rafale` conservée (redirige vers `/admin/quiz?tab=rafale`). Navbar mise à jour : entrées distinctes « Quiz » et « Backstage » dans le groupe Config, entrée « Rafale » retirée. Bouton « NOUVELLE PARTIE » ajouté en interface admin (display-card de GamePage). Aucun contrat modifié — refactor pur frontend.
- **[BREAKING]** **RAFALE now supports multiple categories and difficulties per round, with per-difficulty point scales (#216)** — Le format persisté change : `RAFALE_CATEGORIES` devient liste (`[]string`), `RAFALE_DIFFICULTIES` devient liste (`[]int`), `RAFALE_POINTS_BY_DIFFICULTY` nouvelle map (`map[int]int`) pour un barème libre par niveau de difficulté. Les champs mono `CATEGORY`/`RAFALE_DIFFICULTY` sont conservés en lecture seule et jamais migrés — toute lecture passe par `EffectiveRafaleCategories()`/`EffectiveRafaleDifficulties()` convertissant mono → liste à un élément (rétro-compatible). Tirage unifié pour lancement et cours de manche (traitement homogène de l'épuisement d'un couple) utilisant une **permutation aléatoire du produit cartésien** `catégories × difficultés`, rejouée à l'épuisement. Couple vide ou épuisé sort de la liste active sans bloquer (unicité sur union totalement vide pour START). `GET /api/rafale/pool` passe en paramètres pluriels (ex. `?categories=A,B&difficulties=1,2`). `RafaleCurrent.Points` résout et diffuse le barème de la question courante. Réouverture assumée de la décision du 2026-08-29 (#107) suite retour terrain (voir `contracts/CHANGELOG.md` pour détail technique complet).
- **ENTRACTE becomes 7th question type — second trigger for pause mechanic (#214)** — Nouveau type de question `ENTRACTE` pour les pauses programmées dans le déroulé (en plus du mécanisme ENTRACTE manuel global #119 qui reste inchangé). Chaque occurrence d'ENTRACTE porte sa propre configuration de panneau (titre/sous-titre/image/taille/animation), éditée depuis l'onglet Questions comme tout autre type. Mode déclenchement : entre en `START` du cycle standard (PREPARE → READY → START), restauration automatique de la config globale à la sortie. Aucun point, aucune interaction buzzer/minuteur. Non imbricable en carte MEMOTION (`NestableInMotionCard = false`). Les deux déclencheurs (manuel global + programmé par occurrence) **coexistent intégralement**, jamais l'un ne remplace l'autre — configuration et taille des deux sources restent indépendantes, l'occurrence prioritaire si active. Details complets : `contracts/game-state.md` §"Second déclencheur", `contracts/question-types.md` §7.4.
- **RAFALE becomes nestable in MEMOTION cards (#217)** — Le type RAFALE passe `NestableInMotionCard: true` : les questions RAFALE classiques restent inchangées (questions du déroulé, manche au niveau top), et une nouvelle carte MEMOTION peut désormais être `TYPE=RAFALE` pour embarquer une mini-manche RAFALE à l'intérieur d'une grille MEMOTION polymorphe. Mode **SOLO forcé** pour les cartes RAFALE (pas de sélection d'équipes, 217-Q3). Deux bornes : durée propre carte (`RAFALE_ROUND_TIME`) et nombre de questions (réutilisant le même plafond `RAFALE_MAX_QUESTIONS` que la manche classique). Barème **`STARS_PRORATA`** (comme MEMORY, 217-Q4) — distribution des points sur le nombre d'énoncés trouvés. Réservoir global partagé (une seule base `reservoir.json` pour tous les hôtes RAFALE). État vivant scopé dans `MEMOTION_ACTIVE.STATE` (jamais dans les champs globaux `GameState.RAFALE_*`). Nouvelles actions `RAFALE_VALIDATE`/`RAFALE_INVALIDATE` portant un `MOTION_CARD_ID` optionnel (CardScope, présent uniquement pour les cartes). **Clôt le milestone v9.0.0 (#33) — 5/5 issues complétées et validées.** Détail complet : `contracts/rafale.md` §14.
- **Tab styling enhancement — visual refinement (pastilles + icônes, hors périmètre fonctionnel)** — Amélioration visuelle des onglets (Quiz/Backstage/Rafale) remplaçant le design initial par des pastilles de sélection avec icônes de type. **Changement purement esthétique** — aucune modification fonctionnelle, aucun contrat affecté, aucune évolution du scope des 5 issues du milestone. Les onglets conservent leur comportement existant (navigation, tab parameter via `useSearchParams`). CSS seul.

### Fixed
- **RAFALE gameplay regressions and display issues (#216, QUALIF v9.0.0.4 Batch 1)** — Retour QUALIF v9.0.0.4 : plusieurs régressions du passage multi-catégorie/multi-difficulté (#216) corrigées. Répartition des points entre catégories effectivement tirées au palmarès (`GameEvent.CATEGORY_BREAKDOWN` nouveau, `omitempty`, [BREAKING]) au lieu d'une catégorie absente/"Inconnue". Écrans READY et COUNTDOWN qui affichaient la page PREPARE ou une zone vide — `EffectiveRafaleCategories()`/`EffectiveRafaleDifficulties()` désormais utilisées côté frontend. Libellés des minuteurs clarifiés : « Durée de la manche » vs « Durée par question ». Six sites oubliés (filtres catégorie, équilibre quiz, couleur néon TV, chip animateur, indicateurs admin, résidu) mis à jour pour le format pluriel. Icônes de catégorie sur les chips RAFALE.
- **Type selector overflow — question types (#214) and card types (#217, QUALIF v9.0.0.4 Batch 2)** — Régression #214 : sélecteur de type de question avait une grille figée pour 5 types (3+2 rangées), il y en a 7 depuis #214 → débordement visuel. Même défaut en #217 avec le sélecteur de type de carte MEMOTION (4 boutons avec RAFALE nestable). Corrigé une seule fois : règle CSS `flex-wrap: wrap + min-width: 0` appliquée à `.type-filter-row`/`.type-btn`, sélecteurs dynamiques contre `QUESTION_TYPES` (jamais compte en dur).
- **RAFALE card timer frozen and incorrect animator score (#217, QUALIF v9.0.0.4 Batch 2)** — Retour QUALIF v9.0.0.4 : chrono de question restait figé en carte MEMOTION RAFALE — le protocole `RAFALE_TICK` n'était pas scopé par carte côté client (`MOTION_CARD_ID` était côté serveur mais pas consommé), `useWebSocket.js` patchait toujours le champ global, jamais `MEMOTION_ACTIVE.STATE`. Montant de points affiché à l'animateur incorrect pour une carte RAFALE — `AnimConductPanel.jsx` testait `TYPE === 'MEMORY'` en dur au lieu de vérifier la règle de barème effective (même bug que #187 cycle 4). Libellés des deux sélecteurs de difficulté clarifiés pour carte RAFALE. Placement du chrono en carte harmonisé avec la manche RAFALE classique. Ambiguïté rang/durée levée (« 4ᵉ question » au lieu de « question 4 »).
...
et consolidation de la table des types de questions.

### Added
- **Polymorphic MEMOTION cards with type discrimination** (#184) — Cartes MEMOTION enrichies d'un champ `TYPE` permettant de mixer plusieurs types de questions dans une même manche (QCM, SPEEDY, ARDOISE). Chaque carte spécifie son type et ses règles de scoring propres via le champ `POINTS_RULE`.
- **QCM game type nestable in MEMOTION cards** (#185) — Les questions QCM peuvent désormais être intégrées dans des cartes MEMOTION, avec invalidation des indices lors du décompte du chronomètre pour chaque carte individuellement.
- **Points rule (`POINTS_RULE`) for card-level score control** (#184) — Nouveau champ `POINTS_RULE` permettant de définir une stratégie de calcul de points au niveau de chaque carte MEMOTION — modes `STARS`, `FIXED`, `PER_UNIT` pour adapter le scoring en fonction du type de question et des règles de jeu.
- **Normalized host context (`HostContext` triplet)** (#184) — Composants de question montables dans plusieurs contextes (question directe vs. carte MEMOTION) via un `HostContext` unifié — élimine la duplication de logique de rendu entre les deux contextes d'utilisation.
- **All 5 question types consolidated into single `questionTypeMeta.js` table** (#183) — Table centralisée unifiée pour les métadonnées de tous les types de questions (SPEEDY, QCM, MEMORY, MEMOTION, ARDOISE) — une seule source de vérité pour icônes, libellés et validations.

### Changed
- **MEMOTION card structure refactored for type embedding** (#184) — Structure interne refactorisée pour supporter le champ `TYPE` discriminant dans les cartes MEMOTION — changement transparent pour l'utilisateur, v5.0 → v7.0 interne uniquement.
- **Frontend components no longer receive `phase` prop** (#184, #185) — Composants de question remplacent le prop `phase` par deux booléens explicites `playable` et `revealed` pour un contrat d'interface plus lisible et compatible avec les cartes MEMOTION polymorphes.

---

## [6.5.2] - Milestone v6.5.2 — amélioration visuelle (#19)

**Issues** : #119 (mode ENTRACTE), #152 (backup ambiance), #108 (cartes Memory), #114 (contraste),
#139 (clarification contrat rate-limit), #128 (fuite confidentialité VPlayer). Pause globale
configurable, granularité backup, et correctifs UX/sécurité.

### Added
- **Mode ENTRACTE — pause globale configurable** (#119) — Nouvelle fonctionnalité permettant une pause complète du jeu (déjeuner, changement de salle, annonce) déclenchée depuis l'interface admin. Le booléen `ENTRACTE` dans `GameState` pilote quatre surfaces : TV et écran joueur affichent un **panneau configurable** (titre, sous-titre, image de fond optionnelle) au-dessus du contenu existant estompé (`grayscale(0.85) brightness(0.55)`), l'admin et l'animateur voient leur interface estompée — avec un bouton `ENTRACTE` / `FIN D'ENTRACTE` net et cliquable dans la **Navbar en admin uniquement**. **Garde serveur centralisée** : toutes les actions de jeu (`START`, `REVEAL`, etc.) sont refusées pendant l'entracte ; l'action `ENTRACTE_SET` (admin uniquement) permet l'entrée et la sortie. Configuration **gelée à l'activation** : dès qu'on entre en entracte, les réglages du panneau (titre, sous-titre, image, taille, animation) sont sauvegardés dans `ENTRACTE_CONFIG_SAVED` et ne changent plus tant que l'entracte est actif — les modifications en cours prennent effet au **prochain entracte**. Configuration persistée dans `game_state.json` (propriété de la partie, pas réglage global) avec réglages : titre (défaut `"ENTRACTE"`), sous-titre (défaut `"Retour dans 20mn"`), image de fond optionnelle, taille du panneau `PANEL_SIZE` (défaut 65%, borné 20–100%), animation du panneau (zoom + balancement combinés, durée `ANIM_PERIOD` défaut 10s, amplitude `ANIM_INTENSITY` défaut 20, 0=désactivée). Animation neutralisée sous `prefers-reduced-motion: reduce` (accessibilité). Transition progressive du panneau configurable (entrée/sortie progressive, pas instantanée). Phases autorisées : `STOPPED`, `PREPARE`, `READY`, `NEW_GAME`, `REVEALED` (jamais pendant une manche active — `COUNTDOWN`, `STARTED`, `PAUSED`). LEDs des buzzers éteintes pendant l'entracte, restaurées à la sortie. État **non persisté au redémarrage** (propriété `ShowQRCode` réutilisée comme modèle) — configuration oui, état actif non. Protocole : nouvelle action WebSocket `ENTRACTE_SET {ACTIVE: bool}` (admin uniquement) + `UPDATE_ENTRACTE_CONFIG {config: {...}}` (autorisée même pendant l'entracte, prend effet au prochain cycle). Entièrement additif — aucun champ existant ne change de forme, aucune migration.
- **Sauvegarde/restauration granulaire Configuration Ambiance** (#152) — nouvelle case de sélection indépendante « Configuration Ambiance » dans `/backup-select` et `/reset-select`, permettant de sauvegarder/restaurer/réinitialiser les réglages d'ambiance (fonds d'écran, musique, effets) **indépendamment du flag `history`** — utile pour préserver les réglages lors d'une restauration partielle de jeu.
...
### Fixed
- **Clarification contrat : plafond échecs consécutifs prioritaire sur retry-after** (#139) — le plafond `max_consecutive_failures` prime sur la logique de backoff pour les erreurs 429/413, même si le dernier échec autorisé est lui-même un rate-limit — cela évite une boucle infinie sous rate-limiting persistant. Comportement déjà implémenté, uniquement documenté dans le contrat.
- **Fuite de confidentialité vers l'écran joueur** (#128) — `ARDOISE_ANSWERS` et autres champs réservés à l'admin (`QUIZ_OBJECTIVES`, `FIRMWARE_VERSION`, `OTA_STATUS`, métadonnées buzzer) étaient transmis au VJoueur via sept sites de broadcast (STOP, START, PAUSE, UPDATE_TIMER, etc.). Filtrage désormais systématique **par forme du payload, non par action** : nouvelles listes `AdminOnlyGameFields` et `VPlayerOnlyGameFields` appliquées uniformément sur tous les chemins de diffusion. Nouvelle liste d'exclusion VJoueur : `VPlayerOnlyGameFields = ["ARDOISE_ANSWERS"]` (distinct de `AdminOnlyGameFields`). Aucun impact frontend — aucun client n'affichait ces champs, le défaut portait sur les données transmises sur le réseau.

## [7.1.0] - Milestone v7.1.0 — MEMOTION+ : carte MEMORY & génération IA QCM (#29)

**Issues** : #187, #195, #196. Carte MEMORY imbriquée dans MEMOTION, génération IA pour cartes QCM, et correctifs QUALIF.
...
- **Asymétrie assumée sur les deux sorties de fin de carte MEMORY (#187, cycle 4 QUALIF)** — Le mécanisme d'auto-révélation à l'expiration du chronomètre a été retiré au profit du geste existant `MEMOTION_REVEAL` : grille complètement trouvée avant expiration → révélation immédiate (inchangé) ; chrono expiré avec grille incomplète → carte figée en mode question, l'animateur doit cliquer `MEMOTION_REVEAL` pour révéler (changement d'UX). Correction de la garde de jouabilité via drapeau `motionCardRoundClosed` plutôt que `CURRENT_TIME==0`, évite le piège des cartes sans chrono configuré.
- **Calcul prorata `STARS_PRORATA` branché sur le bouton de crédit (#187, cycle 4)** — L'écran d'animation (`AnimConductPanel`) affiche maintenant le montant exact créditant, non la valeur pleine : pour une carte MEMORY en barème `STARS_PRORATA`, le bouton affiche `points × matched / total` au lieu de `points` pleins. Aucun changement pour QCM/SPEEDY/FIXED/PER_UNIT. Le bandeau d'entête (« ★★ · 5pts ») reste au montant plein avant révélation (affichée en phase `SELECTED` avant toute paire trouvée).
...
- **Défaut de barème `STARS_PRORATA` d'une carte MEMORY (#187, cycle 5 QUALIF v7.1.0.2)** — Une carte MEMORY enregistrée sans `POINTS_RULE` explicite héritait de `STARS` au lieu du défaut du type (`STARS_PRORATA`), créant un écart affichage/crédit observé en revue (5pts/1 paire sur 4 affichait 5 points au lieu de 1). Correctif : résolution du défaut au registre de types `TypeDescriptor.DefaultPointsRule` à chaque lecture (`motionCardPointsForOutcome`) plutôt que par carte — corrige les cartes déjà enregistrées sans re-save, et élimine le piège d'une carte MEMORY→SPEEDY sans jamais écrire `PointsRule` (revalorise aux 5 points pleins, jamais 0).
- **PAUSE/CONTINUER pendant une manche MEMOTION tuait le chrono (#195, cycle 6 QUALIF v7.1.0.3, bug préexistant depuis #151)** — `Engine.Pause()` interrompait définitivement les goroutines des tickers MEMOTION (chrono de carte ET chrono MEMORIZE) au lieu de les mettre en no-op, réduisant l'état à PhaseStarted→PhaseComplete sans possibilité de reprise. `Engine.Continue()` ne relançait jamais rien (goroutines mortes). Correctif : ajouter un branchement `Phase == PAUSED` avant la garde existante, retournant `paused: true` sans arrêter le ticker (même mécanisme que le timer question-hôte qui n'avait jamais eu ce bug). Audit élargi couvre aussi `processMotionMemorizeTick` (mode secret) — même garde vulnérable. Correctif proactif des 2 tickers.
- **Écran TV public cliquable sur les cartes MEMORY (#187, cycle 7 QUALIF v7.1.0.6, bug préexistant)** — La grille MEMORY question-scopée (et reproduit dans la nouvelle grille de carte MEMOTION-MEMORY) ne distinguait pas l'écran TV public (spectateur) de l'aperçu admin en iframe (`/tv?admin=true`) : les deux étaient cliquables via `canClick`. Révélé par ce cycle, jamais introduit par #187. Correctif : exiger `(isVPlayer || isAdminPreview)` en plus des conditions de jeu existantes, sur les **deux** grilles — TV public est désormais entièrement non-interactif, seul l'aperçu admin en iframe reste cliquable.
- **VJoueur ne peut pas retourner de carte MEMORY (#187, cycle 7 QUALIF v7.1.0.6, régression cycle 4)** — `flipMemoryCard` n'envoyait jamais `payload.ID` (identité du bumper), contrairement aux autres actions VJoueur (`ARDOISE_INPUT`/`VPLAYER_QCM_ANSWER`/`BUTTON`). Sans conséquence avant le cycle 4, qui a déplacé la vérification de tour côté serveur — la résolution du bumper échouait silencieusement pour tout VJoueur (3e repli serveur non fiable pour VPlayer), et le flip était ignoré. Correctif : nouveau paramètre `playerId` dans `flipMemoryCard`, transmis par `VPlayerPage` ; comportement additif, aucun changement pour tv/anim qui n'envoient jamais ce paramètre. Les vrais types réels de questions (`QUESTION_TYPES`) restent inchangés.
- **Génération IA MEMOTION+ échoue dès le 1er appel avec « rate limit exceeded » (#196, cycle 2 QUALIF v7.1.0.7)** — Message trompeur : cause réelle était un 413 (requête trop grande) au lieu d'un 429 (quota épuisé). `buildQuestionSchema()` envoyait systématiquement les 6 branches `anyOf` quelle que soit la distribution demandée — gaspillage pré-existant, mais invisible jusqu'à ce que la variante `MEMOTION_PLUS` (union discriminée imbriquée pour QCM) ajoute suffisamment de volume pour franchir le seuil de pré-vérification Groq. Correctif : (1) filtrer le schéma selon la distribution réelle via nouvelle fonction `activeGenerableTypes()` ; (2) distinguer clairement 413 ≠ 429 dans `aiRateLimitError.Error()` pour Groq et Anthropic, en surface le détail du fournisseur au lieu de messages génériques.
- **Génération IA MEMOTION+ échoue avec `json_validate_failed` (#196, cycle 3 QUALIF v7.1.0.8)** — Ambiguïté de discriminateur imbriquée sous `MOTION_CARDS.items.anyOf` (même classe de bug que #142 mais un niveau plus bas) : les deux branches (carte SPEEDY vs carte QCM) partagent un `DIFFICULTY` requis avec le même `enum[1,2,3]`, confondant le validateur Groq sur le discriminateur réel (`TYPE`). Le fix #142 n'avait traité que le chemin racine — `AdaptSchema` navigue désormais **récursivement** tout l'arbre du schéma pour retirer les `enum` candidates à chaque `anyOf` trouvé, quelle que soit la profondeur (MEMOTION classique reste intact : pas d'`anyOf` au niveau de ses `MOTION_CARDS.items`).
- **Génération IA multi-types rate-limited faussement (#196, cycle 3 QUALIF v7.1.0.8)** — `groqRequestMaxTokens` estimait uniquement le prompt, jamais le schéma JSON embarqué dans la même requête, ce qui sous-estime précisément les requêtes multi-types (schéma plus gros = plus de tokens consommés) — biaise le budget `max_tokens` envoyé vers le dépassement 413 silencieux. Correctif : inclure la taille marshalée du schéma dans l'estimation totale (`estimateTokens(prompt) + estimateTokens(schemaJSON)`), même formule calibrée T0.1, assiette corrigée pour refléter le coût réel.

## [8.0.0] - Milestone v8.0.0 — Mode de jeu RAFALE (#16)

**Issues** : #107 (moteur solo) · #197 (réservoir + éditeur) · #198 (interface TV/anim) · #199 (modes multi)

Nouveau mode de jeu « questions en rafale » : sur une manche de ~2 mn, des questions s'enchaînent automatiquement avec ~3 s de réponse chacune. L'animateur valide/invalide manuellement via deux boutons tactiles. Quatre modes : SOLO, CHACUN_SON_TOUR, TANT_QUE_JE_GAGNE, MAILLON_FAIBLE. Les questions proviennent d'un **réservoir global dédié** filtré par catégories (multi) et difficulté (unique), avec marquage persistant des questions utilisées.

### Added
- **Mode de jeu RAFALE — nouveau `QuestionType`** (#107, #197, #198, #199) — Type de question qui porte une **configuration de manche**, pas un énoncé. Les énoncés proviennent d'un réservoir global dédié. RAFALE suit le patron MEMOTION (sous-phases, champs `GameState` dédiés) sans modifier la machine à états principale du jeu (phases STOPPED/PREPARE/READY/COUNTDOWN/STARTED/PAUSED/REVEALED inchangées).
- **Modèle `RafaleQuestion` — réservoir global dédié en fichier unique** (#197) — Structure `{ID, QUESTION, ANSWER, CATEGORY, DIFFICULTY}`, texte seul (pas de média). Réservoir stocké en `data/files/rafale/reservoir.json`, indépendant des questions Quiz/MEMOTION. Persistance typée (`LoadRafale()`/`SaveRafale()`, patron `SaveStatuses`/`LoadStatuses`).
- **Flag "déjà utilisée" — persistant et réinitialisable** (#197) — Fichier `data/config/rafale_used.json`, marque les questions tirées. Persistant à travers redémarrages, réinitialisé automatiquement via `InitGame()` (`NEW_GAME`). Séparé du réservoir (éditer le réservoir ne réécrira pas le flag, jouer ne réécrira pas le réservoir).
- **11 champs `GameState` RAFALE_*** — Sans `omitempty`, initialisés non-nil. Champs : `RAFALE_SUBPHASE` (QUESTION/ROUND_END), `RAFALE_CURRENT_QUESTION` (sans la réponse attendue), `RAFALE_QUESTION_TIME` (décompte ~3 s), `RAFALE_TEAM_COUNTERS` (compteur de bonnes réponses par équipe), `RAFALE_TEAM_BEST` (meilleur compteur, MAILLON_FAIBLE uniquement), `RAFALE_CURRENT_TEAM` (équipe active), `RAFALE_PARTICIPATING_TEAMS` (liste équipes), `RAFALE_CURRENT_TEAM_COLOR` (couleur équipe), `RAFALE_ASKED_COUNT`, `RAFALE_POOL_REMAINING`, `RAFALE_EXHAUSTED`.
- **4 modes de jeu** (#199) — `SOLO` (une seule équipe), `CHACUN_SON_TOUR` (rotation à chaque réponse), `TANT_QUE_JE_GAGNE` (bon=main, mauvais=rotation), `MAILLON_FAIBLE` (idem CHACUN, mais compteur à 0 sur mauvaise réponse, meilleur mémorisé).
- **Pioche randomisée avec filtrage** — Pool = réservoir ∩ catégories choisies ∩ difficulté choisie ∩ non utilisées. Tirage aléatoire uniforme, marquage immédiat, persistance atomique. Comptage du pool avec alerte pré-manche (3 états : bloquant à 0 dispo, avertissement si < besoin estimé, neutre sinon).
- **Double timer simultané** — Timer de manche (réutilise le timer global existant, jusqu'à expiration) + ticker de question dédié (~3 s). Tickers **indépendants**, champs propres (`rafaleQuestionTimer`, `rafaleQuestionStopCh`), jamais `e.timer`/`e.stopCh`. Discipline concurrence stricte : compute sous verrou, callbacks hors verrou, `recoverBackgroundPanic`.
- **3 actions WebSocket entrantes** — `RAFALE_VALIDATE` (réponse correcte), `RAFALE_INVALIDATE` (réponse incorrecte), `RAFALE_SET_TEAMS` (sélection équipes). Admin/anim autorisés sur les deux premières, admin uniquement sur la troisième. Déclarées dans `inbound_allowlist.go` (allow-list fermée, absence = rejet silencieux).
- **2 actions WebSocket sortantes** — `RAFALE_ANSWER` (réponse attendue, **admin + anim uniquement**), `RAFALE_TICK` (décompte timer question, tous). La réponse ne transite **jamais** par `GameState` (précaution fuite de données, classe `ardoise_leak_128`). Diffusion par site d'appel étroit.
- **4 endpoints HTTP JSON** (#197) — `GET /api/rafale/questions` (liste + filtres catégorie/difficulté), `POST /api/rafale/questions` (création/modification), `DELETE /api/rafale/questions/{id}`, `GET /api/rafale/pool` (comptage pré-manche). Corps JSON, pas multipart (pas de média).
- **Page éditeur `/admin/rafale`** (#197, #198) — CRUD complet du réservoir, filtres catégorie/difficulté, affichage état « utilisée », compteurs. Composants réutilisés : CategoryBadge, useCategories, useCategoryFilter, Button, Card.
- **Panneau de configuration de manche** (#198) — Sélection multi-catégories, difficulté unique, mode de jeu, durée de manche, barème de points, temps par question, plafond max questions. Alertes pré-démarrage : bloquage à 0 dispo, avertissement si insuffisant, neutre sinon. Estimation besoin = `ceil(durée / tempsPar)`.
- **Boutons de validation sur `/anim` (L2)** (#198) — Deux boutons tactiles gros : RÉPONSE VALIDE (vert) / RÉPONSE INVALIDE (rouge). Affichent aussi réponse attendue (admin+anim) et équipe active (multi uniquement). Libellés mode-aware (« équipe suivante » masqué en SOLO, etc.).
- **Double timer sur `/tv` et `/anim`** (#198) — Deux décomptes simultanés : manche (~120 s, barre longue) + question (~3 s, barre courte). Design minimaliste, lisible de loin.
- **Vue TV RAFALE — double timer + équipe active + compteurs** (#198) — Affichage question courante (sans réponse), décomptes manche/question, équipe active (mode multi uniquement, bandeau fort), compteurs de chaque équipe (top 3–6 équipes). Statique : `overflow: hidden`, contenu plafonné.
- **Écran de fin de manche** (#198) — Sous-phase `ROUND_END`, l'animateur/admin attribue les points par clic sur une équipe. Valeur pré-remplie = compteur_retenu × barème (voir Scoring ci-dessous). Ajustable avant validation.
- **Scoring : compteur pendant, points en fin** (#199) — Aucun point réel pendant la manche, seul compteur visible. Fin de manche : click équipe → `TEAM_POINTS`, valeur pré-remplie (compteur × points), ajustable. Pas de décrément : aucune pénalité numérique, seule pénalité temporelle. Mode-dependent : MAILLON_FAIBLE crédite le meilleur compteur atteint, autres modes créditent le compteur courant.
- **Indicateur « équipe active » — trois canaux** (#198) — Mode multi uniquement. VPlayer équipe active : élément plein écran gros aux couleurs équipe, passif. TV : bandeau fort nom + couleur équipe active, lisible de loin. **LED des buzzers** : équipe active `SOLID 255`, équipe suivante `SOLID 128`, autres participants `DIM`, absentes OFF. Couleur résolue via `COLOR_NAME`, même grille que MEMORY multi-équipes (factorisation de `sendLEDSetMultiTeam`).
- **Appuis buzzers ignorés pendant RAFALE** — `ProcessButtonPress` sans effet sur état de jeu en type RAFALE (garde précoce).
- **Sauvegarde/restauration sélectives** — Flag `rafale` dans `handleBackupSelect`, `handleResetSelect`, `detectTARContents`, `handleRestore` + checkbox BackupPage.jsx. Réservoir + flag persistants OU absents ensemble (évite perte silencieuse).
- **Réinitialisation automatique du flag au `NEW_GAME`** — Aux côtés des resets MEMOTION/ARDOISE existants, appel `safeGo("SaveRafaleUsed", ...)` dans `InitGame()`.

### Changed
- **Factorisation du helper de rotation d'équipe** — Nouveau helper partagé `rotateTeam()` utilisé par `rotateMotionTeam` (MEMOTION) et `rotateRafaleTeam` (RAFALE). Changement de comportement très subtil sur `MotionCurrentTeamColor` (mise à jour inconditionnelle au lieu de conservation si absent) — cas limite improbable en usage normal (suppression équipe en cours de manche), mais plus sûr (évite afficher couleur périmée). Documenté en contrepartie.

### Fixed
- **Garantie de fuite zéro de la réponse attendue** — Test d'intégration `cmd/server/rafale_answer_leak_test.go` verrouille la diffusion de `RAFALE_ANSWER` à admin+anim uniquement, même via futur copier-colle depuis `broadcastRafaleTick` — précaution contre classe de bug `ardoise_leak_128`.
- **Garde LED dans `sendLEDSetAllBuzzers` pour RAFALE** — Avant correctif, modification équipes pendant manche RAFALE écrasait transitoirement grille LED « équipe active » par DIM générique — auto-corrigé à question suivante. Correctif ajoute branche RAFALE qui délègue à `sendLEDSetRafaleTeams()`.
- **Sous-titres boutons `/anim` mode-aware** — Avant correctif, "équipe suivante" était affiché quel que soit le mode (inexact en SOLO ou TANT_QUE_JE_GAGNE). Correctif : passer `rafaleMode` en prop, adapter libellé selon mode.
- **Réinitialisation de la sélection d'équipes en rejeu (MEMORY, MEMOTION) — cycle 1/2** (#200) — `Engine.Ready()` ne réinitialisait les champs de sélection d'équipes (`Memory*ParticipatingTeams`, `Motion*ParticipatingTeams` et dérivés) que sous condition `isNewQuestion`, jamais lors du rejeu de la même question ID — symptôme identique à #199 (RAFALE), créant une sélection périmée satisfaisant silencieusement la garde `participantsConform`. Correctif : étendre condition à `isNewQuestion || memoryRoundAlreadyPlayed || motionRoundAlreadyPlayed` (signaux capturés avant reset, basés sur champs mutables uniquement sous `Phase==STARTED`). Comportement "persistance lors re-`Ready()` avant tout `Start()`" reste volontairement inchangé (test dédié). Non-régression : trois tests (rejeu Memory/Motion + contrôle négatif persist-before-start pour chaque type).
- **Contournement de participantsConform via Pause()/Continue() — cycle 3** (#200) — Les handlers `Engine.Pause()` et `Engine.Continue()` n'avaient aucune garde de phase, contrairement à `Start()` (#172 B4). `ACTION:"CONTINUE"` pouvait ainsi forcer `Phase==STARTED` directement depuis `PREPARE` (avant toute sélection d'équipe), contournant `participantsConform` dès la première manche MEMORY/MEMOTION — cause racine distincte du cycle 1/2 (rejeu), affectant l'affichage et le comportement. Correctif : ajouter gardes `Continue()` n'accepte que `Phase==PAUSED`, `Pause()` n'accepte que `Phase==STARTED` — même patron que `Start()`/`Reveal()`. Comportement flux légitime inchangé (suite verte confirmée).
- **Broadcast inconditionnels des actions PAUSE/CONTINUE — cycle 4** (#200) — Effet de bord du correctif cycle 3 : les handlers `ActionPause`/`ActionContinue` diffusaient l'action à tous les clients même quand les gardes moteur refusaient la transition, désynchronisant l'affichage de tous les clients connectés via `MSG.GAME.PHASE` codé en dur dans `useWebSocket.js` (avant-que l'affichage frontend ne puisse converger avec l'état réel du serveur). Correctif : conditionner le broadcast à `GetPhase()` réel après l'appel moteur, même patron que `handleStart` (#199) — diffusion nulle si garde refuse, diffusion normale si transition acceptée. Tests cible : reproduction exacte du cycle 3 (MEMORY/zéro équipe/CONTINUE bloqué) → zéro message atteint les clients.
- **Rejeu sans action de jeu (START→STOP sans carte jouée) — cycle 5** (#200) — Signal "manche déjà jouée" du cycle 1/2 reposait exclusivement sur la **progression de jeu** (cartes flippées, paires trouvées, erreurs). Un flux ordinaire (START suivi d'un STOP immédiat sans jamais retourner/sélectionner de carte) laissait la sélection d'équipes de cette première manche EN PLACE côté serveur — rejeu silencieux sur même question ID. Correctif : nouveau signal moteur interne `questionEverStarted` (true dès `Start()`/`StartImmediate()`, reset à false à chaque `Ready()`) remplace les signaux étroits anciens — couvre TOUT cas où la question a au moins atteint l'état STARTED, indépendamment de l'action de jeu suivante (ou absence). RAFALE non affecté : son signal propre (`RafaleSubPhase != None`) dès `Start()` lui garantissait déjà ce comportement.
- **Équipe sélectionnée perdant son dernier buzzer — cycle 6** (#200) — `participantsConform()` vérifie uniquement la **présence de NOMS** d'équipes (`*ParticipatingTeams` non vide), jamais l'existence réelle de buzzers actifs. Une équipe réassignée ailleurs (perdant son unique buzzer) restait comptée comme participante — contournant la garde PREPARE→START/READY/ForceReady indépendamment des cycles 1-5. Affecte MEMORY, MEMOTION ET RAFALE (même défaut structurel). Correctif : fonction `purgeInactiveParticipantUnsafe(teamID)` retire toute équipe de `*ParticipatingTeams` dès qu'elle perd son dernier buzzer, nettoie les champs `*CurrentTeam(*Color)` associés, réévalue la garde PREPARE↔READY — câblée dans `UpdateBumper()` (réassignation buzzer par buzzer) et `SetBumpers()` (réassignation en masse via TeamsPage).
- **Contournement cycle 6 via DELETE_BUMPER (aliasing de map) — cycle 7** (#200) — Handler `handleDeleteBumper` mutait la map live de buzzers **hors du lock** avant de la repasser à `SetBumpers()`, provoquant un aliasing : le diff du cycle 6 comparait des données identiques (même objet en mémoire avant/après), ne détectait aucun changement, et ne déclenchait jamais `purgeInactiveParticipantUnsafe()`. Correctif : nouvelle méthode atomique `Engine.DeleteBumper(id)` effectue delete + diff + purge + réévaluation **sous le même verrou** — élimine l'aliasing et une race indépendante découverte au passage (mutation de map partagée hors verrou). Audit complémentaire : DELETE_BUMPER était le SEUL site avec ce pattern d'aliasing.
- **Seuil de conformité SOLO/multi rationalisé (MEMORY, MEMOTION, RAFALE)** (#201) — Auparavant, les trois types n'appliquaient pas les mêmes règles : MEMORY appliquait SOLO=exactement 1 / multi=au moins 2, MEMOTION exigeait ≥1 sans distinction SOLO/multi (même règle inconditionnelle quel que soit le mode), RAFALE acceptait SOLO=0 et multi=≥1. Bugs structurels : MEMOTION n'avait aucune distinction explicite SOLO/multi (contrairement à MEMORY et RAFALE), RAFALE SOLO acceptait zéro équipe (jamais d'équipe cible pour créditer les points). Correctif : nouveau helper partagé `participantsCountConform(mode, count)` centralise la règle unique — SOLO=exactement 1, multi=au moins 2 — appliquée uniformément à tous les trois types. Frontend : affichage du motif d'attente en PREPARE aligné (`prepareWaitReason.js`). Changement de comportement utilisateur visible : MEMOTION multi exige désormais exactement 1 équipe en SOLO et ≥2 en multi (avant ≥1 inconditionnel), RAFALE SOLO exige ≥1 équipe (avant 0 accepté, jamais de cible). Non-régression : sélections multiples testées.

### Added
- **Pré-tirage et zone SUIVANTE pour RAFALE** (#202) — Sur `/anim`, encart RAFALE enrichi d'une zone « SUIVANTE » en bas affichant l'énoncé, la catégorie et la difficulté de la prochaine question — vue prévisualisée d'une question réellement pré-tirée par le moteur (pas un aperçu jetable). Moteur : nouveau champ privé `Engine.rafaleNext` contenant la question suivante, tiré lors du démarrage de chaque question et consommé au tick suivant ; libération exhaustive en fin de manche, STOP ou plafond atteint. Protocole : extension additive du champ `NEXT` sur l'action `RAFALE_ANSWER` existante (admin+anim uniquement, jamais dans `GameState` — même régime de confidentialité que la réponse attendue), portant `{ID, QUESTION, CATEGORY, DIFFICULTY}` sans le champ `ANSWER` ; champ diffusé aux seuls `ClientTypeAdmin` et `ClientTypeAnim`, garantie anti-fuite bloquante. Frontend : affichage `AnimRafaleQuestion` adapté avec double-zone lisibilité visuelle (question courante ~2,5× plus grande que zone SUIVANTE) — **réserve non-bloquante** : ce ratio peut se dégrader sur petit viewport, à valider par l'utilisateur. Aucun changement BREAKING.

**Récapitulatif #200 — six causes racines du symptôme "START possible sans sélection"** :
1. Cycle 1/2 : sélection périmée sur rejeu (Ready → fix condition)
2. Cycle 3 : contournement direct via Pause/Continue (gardes phase)
3. Cycle 4 : broadcast non gardés (frontend désync)
4. Cycle 5 : rejeu zéro-action (Start→Stop sans carte jouée)
5. Cycle 6 : équipe perdant son dernier buzzer (réassignation)
6. Cycle 7 : DELETE_BUMPER contournant purge (aliasing de map)

**Aucun changement BREAKING** : tous les ajouts sont additifs. Phases existantes, timer global, `TEAM_POINTS`, charges `/tv`/`/player`/`/anim`, buzzers restent inchangés à périmètre constant (voir contrats détaillés en `contracts/CHANGELOG.md` et `contracts/rafale.md`).

**Contrats détaillés** : `contracts/rafale.md` (spécification complète, décisions d'architecture, checklist d'accroche, non-régression exigée), `_work/reports/plan-20260828-161558.md` (composants impactés).
...
- **Philips Hue Bridge REST driver (API v1, replaces abandoned BLE path)** (#206) — Pilote implémentant `lighting.Driver` pour ampoules Hue connectées via bridge BLE. **Découverte mDNS + SSDP en 0,2 s, sans IP fixe ni appel cloud** ; identité bridge par `bridge_id` (immuable) et `bridge_ip` (auto-mise à jour DHCP) ; ampoules par **nom** (jamais par entier réattribué), résolution au démarrage et chaque 5 min. **Débit recommandé : 250 ms** (mesuré réel 19-61 ms, séquentiel séries courtes ≤ 6 ampoules). Écriture seulement des changements d'état (couleur, intensité). Dégradé fin-grained : pont injoignable → backoff 1-60 s ; clé refusée → impossible ; ampoule inconnue → non-événement. Test exhaustif sans matériel (faux pont `httptest`, latence simulée). `server-go/internal/lighting/hue/` : `discover.go` (mDNS/SSDP), `guard.go` (requêtes whitelistées), `client.go` (v1 GET/POST minimal), `color.go` (RGB → CIE xy), `driver.go` (interface `lighting.Driver`).

- **Documentation utilisateur et technique pour Hue Bridge (v10.0.0, Batch 4)** — `docs/ADMIN_GUIDE.md` enrichie de trois sections : (1) **Sélecteur tri-état** (ON/AUTO/OFF) avec règle critique « ON/OFF tiennent indéfiniment » et seul AUTO rend la main au jeu ; (2) **Ampoules d'équipe** ne sont jamais affectées par le sélecteur général (préparation #213) ; (3) **Arrêt serveur** → extinction totale, **perte connexion** → resync automatique au retour. `docs/SERVER_PARAMETERS.md` nouveau : section `lighting` normative documentant `enabled`, `bridge_ip`, `bridge_id`, `lights[]` (nom/rôle/team), débit (250 ms), timeouts, reconnexion backoff, résolution des noms, gestion des secrets (clé API jamais persistée), comportement dégradé. `docs/LED_SET_PROTOCOL.md` §1 nouveau : distinction clarifiée entre LED buzzers (WebSocket per-buzzer, ce protocole) et ambiance Hue (HTTP vers bridge, contrat `contracts/lighting.md`). **Documentation technique externe** : `contracts/lighting.md` (v10.0.0, §10 : design tri-état + retour manuel fermé au GATE 2026-09-07), `contracts/hue-bridge.md` (§5.1-§5.7 : architecture pilote, conversions couleur, dégradation par équipe, cycles de vie).

- **Admin page for Hue Bridge configuration** (#207) — Nouvelle page `/admin/ambiance` avec trois étapes séquentielles : (1) **Découverte automatique** (mDNS/SSDP) ou saisie IP de repli ; (2) **Appui bouton requis** (compte à rebours 45 s, relances 2 s, aucun champ clé) ; (3) **Sélection ampoules** (case à cocher par ampoule, tout coché par défaut, liste figée si pont injoignable). État de connexion du pont en badge : ✅ ok (vert) / ⚠️ injoignable (rouge) / ❌ association refusée (orange) / ⏸️ non configuré (gris). Glyphes : pastille/rayons pour actif, pastille/vide pour alerte, vide pour arrêt. Renommage simultané : « Configuration Ambiance » (backup) → « Réglages de jeu » (clarté utilisateur sur ce qui est sauvegardé).

- **Team-differentiated lighting prepared for #213** (#206, #207) — Architecture prête pour colorer une zone par équipe dès que #213 ajoute `role: "team"` aux configurations d'ampoule. Zone `general` et zones par équipe coexistent (contrat `contracts/lighting.md` §3, `ZoneState`). À v10.0.0, une seule zone « general » utilisée ; champ `role` ignoré jusqu'à #213.

### Changed

- **Security fix : SSRF protection on Hue Bridge registration** (#206) — Validation stricte des adresses de pont avant tout appel sortant : `validateBridgeAddress` résout les noms d'hôte et exige que **toutes** les adresses soient privées/link-local (RFC 1918, 169.254/16, loopback). Rejette non-http(s), userinfo, chemin, query, fragment, contrôle. Application dans `handleLightingRegister` (avant `hue.Register`) et dans `/config.json` (avant persistance). Test `TestDevLightingRegisterRefusesPublicTargetWithoutNetwork` ✅ en < 500 ms sans appel sortant. **Pas de réflexion du corps cible** : `writeLightingError` log en serveur, répond `{"result":"error"}` au client (aucun `err.Error()`). **Clé validée avant sauvegarde** : `validHueAPIKey` appliquée avant persistance ; échec → 502 (jamais logguée, seule sa longueur l'est).

- **Rate-limit guard on concurrent lighting operations** (#206) — Opérations longues (register, discover, test) sérialisées via `lightingInFlight` atomic : deuxième appel concurrent reçoit `429 busy` **sans aucun échange réseau** avec le pont (prévention du SSRF résiduel en scanner). Test `TestDevLightingRegisterConcurrentIsSerialised` ✅ prouve pont touché une fois, second appelant reçoit 429.
