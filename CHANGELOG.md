...
- **Seuil de conformité SOLO/multi rationalisé (MEMORY, MEMOTION, RAFALE)** (#201) — Auparavant, les trois types n'appliquaient pas les mêmes règles : MEMORY appliquait SOLO=exactement 1 / multi=au moins 2, MEMOTION acceptait zéro équipe dès la première manche, RAFALE acceptait SOLO=0 et multi=≥1. Bugs : points RAFALE SOLO ne pouvaient jamais être attribués (aucune équipe cible), MEMOTION déclarée "conforme" sans équipe sélectionnée (contournant les gardes PREPARE→READY). Correctif : nouveau helper partagé `participantsCountConform(mode, count)` centralise la règle unique — SOLO=exactement 1, multi=au moins 2 — appliquée uniformément. Frontend : affichage du motif d'attente en PREPARE aligné (`prepareWaitReason.js`). Changement de comportement utilisateur visible : MEMOTION et RAFALE multi exigent désormais ≥2 équipes (avant ≥1), RAFALE SOLO exige ≥1 équipe (avant 0 accepté). Non-régression : sélections multiples testées.
