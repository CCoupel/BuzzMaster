...
### Added
- **Mode ENTRACTE — pause globale configurable** (#119) — Nouvelle fonctionnalité permettant une pause complète du jeu (déjeuner, changement de salle, annonce) déclenchée depuis l'interface admin. Le booléen `ENTRACTE` dans `GameState` pilote quatre surfaces : TV et écran joueur affichent un **panneau configurable** (titre, sous-titre, image de fond optionnelle) au-dessus du contenu existant estompé (`grayscale(0.85) brightness(0.55)`), l'admin et l'animateur voient leur interface estompée — avec un bouton `ENTRACTE` / `FIN D'ENTRACTE` net et cliquable en admin uniquement. **Garde serveur centralisée** : toutes les actions de jeu (`START`, `REVEAL`, etc.) sont refusées pendant l'entracte ; l'action `ENTRACTE_SET` (admin uniquement) permet l'entrée et la sortie. Configuration persistée dans `game-config.json` (section `entracte`) avec réglages : titre (défaut `"ENTRACTE"`), sous-titre (défaut `"Retour dans 20mn"`), image de fond optionnelle, taille du panneau `PANEL_SIZE` (défaut 65%, borné 20–100%), animation du panneau (zoom + balancement combinés, durée `ANIM_PERIOD` défaut 10s, amplitude `ANIM_INTENSITY` défaut 20, 0=désactivée). Animation neutralisée sous `prefers-reduced-motion: reduce` (accessibilité). Phases autorisées : `STOPPED`, `PREPARE`, `READY`, `NEW_GAME`, `REVEALED` (jamais pendant une manche active — `COUNTDOWN`, `STARTED`, `PAUSED`). LEDs des buzzers éteintes pendant l'entracte, restaurées à la sortie. État **non persisté au redémarrage** (propriété `ShowQRCode` réutilisée comme modèle) — configuration oui, état actif non. Protocole : nouvelle action WebSocket `ENTRACTE_SET {ACTIVE: bool}` (admin uniquement). Entièrement additif — aucun champ existant ne change de forme, aucune migration.
