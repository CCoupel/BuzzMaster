...
- **Réinitialisation de la sélection d'équipes en rejeu (MEMORY, MEMOTION)** (#200) — `Engine.Ready()` ne réinitialisait les champs de sélection d'équipes (`Memory*ParticipatingTeams`, `Motion*ParticipatingTeams` et dérivés) que sous condition `isNewQuestion`, jamais lors du rejeu de la même question ID — symptôme identique à #199 (RAFALE), créant une sélection périmée satisfaisant silencieusement la garde `participantsConform`. Correctif : étendre condition à `isNewQuestion || memoryRoundAlreadyPlayed || motionRoundAlreadyPlayed` (signaux capturés avant reset, basés sur champs mutables uniquement sous `Phase==STARTED`). Comportement "persistance lors re-`Ready()` avant tout `Start()`" reste volontairement inchangé (test dédié). Non-régression : trois testes (rejeu Memory/Motion + contrôle négatif persist-before-start pour chaque type).
