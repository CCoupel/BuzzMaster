...
- **Réinitialisation de la sélection d'équipes en rejeu (MEMORY, MEMOTION)** (#200) — `Engine.Ready()` ne réinitialisait pas les champs de sélection d'équipes (`Memory*ParticipatingTeams`, `Motion*ParticipatingTeams` et dérivés) lors du rejeu de la même question — symptôme identique à #199 (RAFALE), créant une sélection périmée satisfaisant silencieusement la garde `participantsConform`. Correctif : généralisation du fix #199 à MEMORY et MEMOTION, réinitialisation systématique de tous les champs de participation sur appel `Ready()`. Non-régression : tests couvrent les trois modes (solo/multi/MEMOTION) sur rejeu consécutif.
